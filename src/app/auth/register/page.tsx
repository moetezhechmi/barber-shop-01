...
import { Scissors, Eye, EyeOff, Briefcase, Store, Sparkles, ArrowRight } from "lucide-react";
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#070708] px-4 font-sans sm:px-6 lg:px-8 py-10">
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="backdrop-blur-3xl bg-neutral-900/40 border border-neutral-800 rounded-[32px] p-8 sm:p-10 shadow-2xl transition-all duration-500">
          ...
          <div className="text-center mb-8">
            <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-500 to-indigo-600 shadow-lg shadow-violet-500/20 border border-white/10 transform hover:scale-105 transition-transform duration-300">
              {role === "CLIENT" && <Scissors className="h-6 w-6 text-white" />}
              {role === "OWNER" && <Store className="h-6 w-6 text-white" />}
              {role === "BARBER" && <Briefcase className="h-6 w-6 text-white" />}
            </div>
            
            <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white mb-2 flex items-center justify-center gap-2">
              {role === "CLIENT" && "Créer un compte"}
              {role === "OWNER" && "Espace Gérant"}
              {role === "BARBER" && "Espace Barbier"}
              <Sparkles className="h-4 w-4 text-violet-400" />
            </h1>
            
            <p className="text-sm text-neutral-400">
              {role === "CLIENT" && "Réservez vos prestations en un clic"}
              {role === "OWNER" && "Gérez votre salon et votre équipe"}
              {role === "BARBER" && "Rejoignez un salon et gérez vos rendez-vous"}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Dynamic Role Switch Footer Links */}
            <div className="pt-6 border-t border-neutral-800/80 text-center space-y-3">
              {role === "CLIENT" ? (
                <p className="text-xs text-neutral-400">
                  ...
                  <button
                    type="button"
                    onClick={() => { setError(null); setRole("OWNER"); }}
                ...
                <p className="text-xs text-neutral-400">
                  ...
                  <button
                    type="button"
                    onClick={() => { setError(null); setRole("CLIENT"); }}
                    className="font-bold text-violet-400 hover:text-violet-300 transition-colors"
                  >
